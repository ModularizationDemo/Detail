    ...
    _fetchNewsDetailCommand = [[RACCommand alloc]initWithSignalBlock:^RACSignal *(id input) {
        return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
            ...
            [self requestForNewsDetailSuccess:^(NSDictionary *result) {
                ...
                self.detailModel = [SXNewsDetailEntity detailWithDict:result[self.docid]];
                if (self.boardid.length < 1) {
                    self.boardid = self.detailModel.replyBoard;
                }
                self.replyCount = @(self.detailModel.replyCount);
                
                self.sameNews = [NSArray yy_modelArrayWithClass:[SXSimilarNewsEntity class] json:result[self.docid][@"relative_sys"]];
                self.keywordSearch = result[self.docid][@"keyword_search"];
